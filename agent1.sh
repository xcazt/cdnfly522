...
if [ "$choice" == "1" ]; then
    ...
    echo -e "\033[0;32m正在安装工具...\033[0m"
    sudo yum install chrony -y
    sudo systemctl start chronyd
    sudo systemctl enable chronyd
    echo -e "\033[0;32m正在修复节点Bug\033[0m"
...
if [[ $MASTER_VER == "" ]]; then
    ...
    if [[ "$agent_ver" == "" ]]; then
        echo "无法获取agent版本，使用指定的AGENT_VER: $AGENT_VER"
        dir_name="cdnfly-agent-$AGENT_VER"
        tar_gz_name="$dir_name-$(get_sys_ver).tar.gz"
    else
...
download "https://github.com/xcazt/cdnfly522/raw/refs/heads/main/$tar_gz_name" "https://github.com/xcazt/cdnfly522/raw/refs/heads/main/$tar_gz_name" "$tar_gz_name"
